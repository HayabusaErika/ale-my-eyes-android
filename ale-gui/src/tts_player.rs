...
fn play_audio_android(audio_data: &[u8]) -> Result<(), String> {
    ...
    let _cleanup = TempFileCleanup(temp_file.clone());
    ...
    let duration_ms = env
        .call_method(&mp, "getDuration", "()I", &[])
        .ok()
        .and_then(|value| value.i().ok())
        .unwrap_or(0)
        .max(0) as u64;
    let timeout = std::time::Duration::from_millis(duration_ms.saturating_add(5_000));
    let started = std::time::Instant::now();

    loop {
        let is_playing = env
            .call_method(&mp, "isPlaying", "()Z", &[])
            .ok()
            .and_then(|value| value.z().ok())
            .unwrap_or(false);
        if !is_playing || started.elapsed() >= timeout {
            break;
        }
